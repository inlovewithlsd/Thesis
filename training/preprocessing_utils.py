...
def replace_preds(sparql, preds_dict):
    preds = re.findall(r'\b(?:wdt|p|ps):P\d+\b', sparql)
    ...
    replacement = {'instance_of': 'P31', 'subclass_of': 'P279'}

    for pred in preds:
        prefix, clean_pred = pred.split(':')
        if clean_pred in preds_dict and preds_dict[clean_pred]['label']:
            pred_desc = "_".join(preds_dict[clean_pred]['label'].split())
            reformatted_sparql = reformatted_sparql.replace(pred, prefix + ':' + pred_desc)
            replacement[pred_desc] = clean_pred

    return reformatted_sparql, replacement

def process_predicate(label_str):
    label_str = "_".join(label_str.split())
    return label_str
